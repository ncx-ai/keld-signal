...
fetch_sidecar() {
  ...
  if [ -n "$want" ]; then
    ...
    if [ "$want" != "$got" ]; then
      echo "  ! analysis sidecar CHECKSUM MISMATCH — download corrupt or truncated; not installed." >&2
  ...
  tar -xzf "${work}/${asset}" -C "${work}/x" || {
    echo "  ! analysis sidecar archive could not be unpacked; not installed." >&2
    return 1
  }
  [ -d "${work}/x/keld-agent-sidecar" ] || {
    echo "  ! analysis sidecar archive had an unexpected layout; not installed." >&2
    return 1
  }
  ...
  if ! mv "${work}/x/keld-agent-sidecar" "${dest}/keld-agent-sidecar"; then
    ...
    echo "  ! analysis sidecar could not be moved into place; not installed." >&2
    return 1
  fi
  rm -rf "${dest}/keld-agent-sidecar.prev"
