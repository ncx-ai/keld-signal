...
// remoteWorkstreams is the org's current values, installed once the projects store
...
var remoteWorkstreams atomic.Pointer[func() []settings.RemoteWorkstream]

func setRemoteWorkstreams(fn func() []settings.RemoteWorkstream) {
	if fn == nil {
		remoteWorkstreams.Store(nil)
		return
	}
	remoteWorkstreams.Store(&fn)
}
...
func workstreamMatchesFor(b enrich.BlockCharacterisation) []publish.WorkstreamMatch {
	...
	candidates := d.Workstreams
	if fn := remoteWorkstreams.Load(); fn != nil {
		if remote := workstreams.FromRemoteWorkstreams((*fn)()); len(remote) > 0 {
			candidates = workstreams.MergeCandidates(candidates, remote)
		}
	}
	matches := workstreams.MatchesFor(b.Analysis.Dimensions, candidates,
		workstreams.GroupOffFunc(settings.Load()))
	if len(matches) == 0 {
		return nil
	}
	out := make([]publish.WorkstreamMatch, 0, len(matches))
	for _, e := range matches {
		out = append(out, publish.WorkstreamMatch{
