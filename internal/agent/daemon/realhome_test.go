...
// migrates state/projects.json to workstreams.json on the way up. So `go test
// ./...` on a working machine performed that migration on its real state: it
// wrote an empty workstreams.json beside a live projects.json, and the next
// build of the daemon read the empty one — every workstream the person had made
