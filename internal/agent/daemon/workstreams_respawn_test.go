...
type recordingPoster struct {
	mu    sync.Mutex
	posts [][]settings.RemoteWorkstream
	fail  error
}

func (r *recordingPoster) post(p []settings.RemoteWorkstream) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	if r.fail != nil {
		return r.fail
...
func TestARespawnRePostsTheWorkstreamListEvenThoughNothingChanged(t *testing.T) {
	list := []settings.RemoteWorkstream{{ID: "proj_a", Title: "A"}}
	state := &workstreamsState{}
	rec := &recordingPoster{}

	// Normal startup: the list is resolved, believed, and posted once.
	postWorkstreamsIfKnownNonEmpty(rec.post, state, workstreamsResolution{list: list, ok: true}, "%v")
	if rec.count() != 1 {
	...
	repostWorkstreamsAfterRespawn(rec.post, state)
	if rec.count() != 2 {
...
// exact clobber postWorkstreamsIfKnownNonEmpty's empty-skip guard exists to stop.
func TestARespawnPostsNothingWhenNoWorkstreamsWereEverDeclared(t *testing.T) {
	rec := &recordingPoster{}
	repostWorkstreamsAfterRespawn(rec.post, &workstreamsState{})
	if rec.count() != 0 {
		t.Fatalf("nothing was ever declared; got %d posts", rec.count())
	}
	// And a nil poster (no sidecar this run) must not panic.
	repostWorkstreamsAfterRespawn(nil, &workstreamsState{})
}

// A FAILED re-post must not be remembered as success — the same rule
// workstreamsState.set already follows — so the next respawn (or the next real
// change) tries again.
func TestAFailedRePostLeavesTheStateAbleToTryAgain(t *testing.T) {
	list := []settings.RemoteWorkstream{{ID: "proj_a"}}
	state := &workstreamsState{}
	rec := &recordingPoster{fail: errors.New("sidecar unreachable")}
	state.observe(workstreamsResolution{list: list, ok: true})
...
func TestTheDaemonBelievesItsWorkstreamListBeforeTheFirstPostLands(t *testing.T) {
	state := &workstreamsState{}
	...
	if !state.changed([]settings.RemoteWorkstream{{ID: "proj_a"}}) {
...
// same read-error-is-not-an-empty-list rule workstreamsResolution exists for, and
...
func TestModelDemandIsGatedOnAKnownNonEmptyWorkstreamList(t *testing.T) {
