...
// fakeBlocks is a minimal workstreams.BlocksSource for these tests: a fixed set
// of block summaries, standing in for the ledger store (D2) this package must
// not depend on.
type fakeBlocks struct{ rows []workstreams.BlockSummary }

func (f fakeBlocks) SinceWeekStart() ([]workstreams.BlockSummary, error) { return f.rows, nil }

func attributedDim(v string) enrich.Labeled {
	return enrich.Labeled{Value: v, Confidence: 1, Status: enrich.DimensionAttributed}
}

func newTestStore(t *testing.T) *workstreams.Store {
	t.Helper()
	t.Setenv("KELD_HOME", t.TempDir())
	return workstreams.NewStore(filepath.Join(t.TempDir(), "projects.json"))
}
...
func TestWorkstreamsRoutesRequireTheSecret(t *testing.T) {
	s := newTestStore(t)
...
func TestGetWorkstreamsOnEmptyStoreIsHonestlyEmpty(t *testing.T) {
	s := newTestStore(t)
	srv := httptest.NewServer(DiscardHandler("s3cret", WorkstreamsRoute(s)))
	defer srv.Close()
...
func TestGetWorkstreamsComputesSuggestionsFromInjectedBlocksSource(t *testing.T) {
	s := newTestStore(t)
...
func TestGroupOffRouteWritesSettingsAndIsLocalOnly(t *testing.T) {
	...
	// attribution.
	doc := workstreams.Document{
		Groups: []workstreams.Group{{Key: "marketing", Name: "Marketing"}},
		Workstreams: []workstreams.Workstream{
...
func TestPlaceRefusesUnknownSuggestion(t *testing.T) {
	s := newTestStore(t)
	if err := s.Save(workstreams.Document{Workstreams: []workstreams.Workstream{{ID: "p1", Title: "One"}}}); err != nil {
		t.Fatal(err)
	}
	srv := httptest.NewServer(DiscardHandler("s3cret", WorkstreamsRoute(s)))
	defer srv.Close()
...
func TestGetWorkstreamsRulesOnlyShowMatchedKeywords(t *testing.T) {
	s := newTestStore(t)
	if err := s.Save(workstreams.Document{Workstreams: []workstreams.Workstream{
