...
func TestT1LocalFullyCoveredIsDeletedAndItsBlocksStillAttribute(t *testing.T) {
	...
	next, removed, trimmed := Reconcile(d, remote)
	...
	got := Attribute(repoDims("github.com/acme/a"), MergeCandidates(next.Projects, remote), nil)
...
func TestT2CoveredByTwoOrgProjectsIsStillDeleted(t *testing.T) {
	...
	_, removed, _ := Reconcile(d, remote)
	if len(removed) != 1 {
...
func TestT4AfterTrimmingAnOverlappingRuleIsNoLongerDoubleCounted(t *testing.T) {
	...
	before := Attribute(repoDims("github.com/acme/a"), MergeCandidates(d.Projects, remote), nil)
	if len(before.Projects) != 2 {
		t.Fatalf("precondition: want both assigned before the trim, got %+v", before.Projects)
	}

	next, _, _ := Reconcile(d, remote)
	after := Attribute(repoDims("github.com/acme/a"), MergeCandidates(next.Projects, remote), nil)
...
func TestT5TheRemainderStillAttributes(t *testing.T) {
	...
	next, _, _ := Reconcile(d, remote)
	got := Attribute(repoDims("github.com/acme/c"), MergeCandidates(next.Projects, remote), nil)
...
func TestT8ReconcileIsIdempotent(t *testing.T) {
	...
	once, _, trimmed1 := Reconcile(d, remote)
	if len(trimmed1) != 1 {
		t.Fatalf("first pass trimmed %d", len(trimmed1))
	}
	twice, removed2, trimmed2 := Reconcile(once, remote)
...
func TestT9AHiddenOrgProjectCoversNothing(t *testing.T) {
	// ⚠️ It is excluded from matching entirely, so counting its rules as
	// coverage would delete a local project and leave its blocks in NO project
	// — the opposite of what coverage guarantees. (Was: a group switched off;
	// Revision 4 made hidden the only exclusion.)
	d := doc(localProject("p_a", "mine", "github.com/acme/a"))
	remote := FromRemoteProjects([]settings.RemoteProject{
	...
	next, removed, trimmed := Reconcile(d, remote)
	if len(removed) != 0 || len(trimmed) != 0 {
		t.Fatalf("a hidden org project was treated as coverage: removed=%+v trimmed=%+v",
			removed, trimmed)
	}
...
func TestT11NoOrgProjectsChangesNothing(t *testing.T) {
	...
	d := doc(localProject("p_a", "mine", "github.com/acme/a"))
	next, removed, trimmed := Reconcile(d, nil)
...
func TestAHiddenLocalProjectIsLeftAlone(t *testing.T) {
	...
	_, removed, trimmed := Reconcile(d, remote)
	if len(removed) != 0 || len(trimmed) != 0 {
