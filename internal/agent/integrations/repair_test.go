...
func liveParams() tools.SetupParams {
	return tools.SetupParams{Endpoint: "http://127.0.0.1:14318", IngestToken: "local-secret", BinPath: "/usr/local/bin/keld"}
}
...
func TestARepairedRowSaysSoUnderRestartRequired(t *testing.T) {
	...
	in := computeOne(now, DefaultWindow, e, f)
...
func TestAnOldRepairAgesOutButNotWhileTheRestartIsStillOutstanding(t *testing.T) {
	...
	if got := computeOne(now, DefaultWindow, e, idle); got.State != Idle {
	...
	got := computeOne(now, DefaultWindow, e, stale)
