...
const (
	...
	// ReasonOTLPSwitch — the config disagrees with the `tool_otlp` switch: the
	// block is present and the switch is off, or absent and the switch is on.
	// Not a fault in the machine, which is why it carries no repair note.
	ReasonOTLPSwitch = "otlp_switch"
)
