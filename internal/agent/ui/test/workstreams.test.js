...
import { findConflicts, reasonText, workstreamTitle, workstreamCellInfo } from "../app.js";
...
// --- workstreamTitle / workstreamCellInfo: the Today table's Project column must
...
const projectsPayload = { workstreams: [{ id: "p_keld_signal", title: "Keld Signal" }, { id: "p_keld_atlas", title: "Keld Atlas" }] };
...
test("workstreamTitle resolves a known id to its title, never the id itself", () => {
  assert.equal(workstreamTitle("p_keld_signal", projectsPayload), "Keld Signal");
});

test("workstreamTitle returns null for an id the projects list doesn't carry", () => {
  assert.equal(workstreamTitle("p_does_not_exist", projectsPayload), null);
});

test("workstreamTitle returns null for an empty/absent id", () => {
  assert.equal(workstreamTitle("", projectsPayload), null);
  assert.equal(workstreamTitle(undefined, projectsPayload), null);
});

test("workstreamCellInfo: an attributed block with a resolvable id shows the TITLE, styled as a real project", () => {
  const info = workstreamCellInfo(attributedBlock("p_keld_signal", "repo"), projectsPayload);
...
test("workstreamCellInfo: an id with NO match falls back to the id, but is flagged unresolved (not attributed)", () => {
  const info = workstreamCellInfo(attributedBlock("p_ghost_project", "repo"), projectsPayload);
...
test("workstreamCellInfo: a block that ran attribution but matched no rule is 'none', not 'unresolved'", () => {
  const info = workstreamCellInfo(attributedBlock(""), projectsPayload);
  assert.equal(info.kind, "none");
});

test("workstreamCellInfo: a block whose attribution stage never ran is 'unknown', absent rather than a guess", () => {
  const info = workstreamCellInfo({ key: { session: "s1", start: 0 }, end: 60, cells: {} }, projectsPayload);
