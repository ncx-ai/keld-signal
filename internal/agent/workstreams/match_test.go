...
func TestT13BothSidesOfTheComparisonAreOnTheRow(t *testing.T) {
	...
	local := localWorkstream("p_abc", "mine",
	...
	got := MatchesFor(repoDims("github.com/acme/a"),
		MergeCandidates([]Workstream{local}, remote), noneOff)
...
func TestT14WorkstreamMatchesCarryNoLocalIdentity(t *testing.T) {
	// ⚠️ A local project's ID IS DERIVED FROM ITS TITLE (newWorkstreamID), so
	...
	got := MatchesFor(repoDims("github.com/acme/a"), []Workstream{local}, noneOff)
	if len(got) != 1 {
...
func TestMatchRulesAreSortedSoTwoMachinesAgree(t *testing.T) {
	...
	a := localWorkstream("p_1", "one", "github.com/acme/b", "github.com/acme/a")
	b := localWorkstream("p_2", "two", "github.com/acme/a", "github.com/acme/b")
	ea := MatchesFor(repoDims("github.com/acme/a"), []Workstream{a}, noneOff)
	eb := MatchesFor(repoDims("github.com/acme/a"), []Workstream{b}, noneOff)
...
func TestWorkstreamMatchesListEveryMatchRatherThanPickingOne(t *testing.T) {
	...
	both := []Workstream{
		localWorkstream("p_a", "mine", "github.com/acme/a"),
...
func TestAMatchCanComeFromATicketKeyInTheBranch(t *testing.T) {
	p := Workstream{ID: "org:one", Title: "One", Origin: OriginAtlas,
	...
	got := MatchesFor(dims, []Workstream{p}, noneOff)
