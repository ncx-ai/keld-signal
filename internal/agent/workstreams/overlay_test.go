...
// ErrWorkstreamNotFound. Found by reading the code against the decision, not by a
...
func TestSameAsOntoAnAtlasValueCreatesALocalOverlayAndAttributes(t *testing.T) {
	...
	before := Attribute(blockOn("github.com/ncx-ai/keld-cli"), MergeCandidates(d.Workstreams, remote), off, nil)
	if before.WorkstreamID != "" {
	...
	if len(next.Workstreams) != 1 || next.Workstreams[0].ID != "keld_projects:signal" || next.Workstreams[0].Origin != OriginAtlas {
		t.Fatalf("an overlay entry keyed by the Atlas id must be created, got %+v", next.Workstreams)
	}
	...
	merged := MergeCandidates(next.Workstreams, remote)
	after := Attribute(blockOn("github.com/ncx-ai/keld-cli"), merged, off, nil)
	if after.WorkstreamID != "keld_projects:signal" || after.Method != MethodRepo {
...
func TestSameAsRefusalsStillHold(t *testing.T) {
	...
	if _, err := PlaceSameAsWithRemote(Document{}, remote, sugs[0].ID, "nope", sugs, func(string) bool { return false }); err != ErrWorkstreamNotFound {
		t.Fatalf("unknown target must be ErrWorkstreamNotFound, got %v", err)
	}
