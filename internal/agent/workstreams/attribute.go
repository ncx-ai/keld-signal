...
type Vector interface {
	...
	Attribute(dims map[string]enrich.Labeled, candidates []Workstream) Result
}
...
func DeclaredRepos(p Workstream) []string {
...
func CandidateRepos(p Workstream) []string {
	var out []string
...
func EffectiveRepos(p Workstream) []string {
...
// blocks (see internal/agent/ingress/workstreams.go's GET handler). A candidate
...
func MatchedCandidates(p Workstream, observedRepos []string) []string {
	var out []string
...
func Rules(p Workstream, observedRepos []string) []string {
...
func Visible(candidates []Workstream, groupOff func(key string) bool) []Workstream {
	out := make([]Workstream, 0, len(candidates))
	for _, p := range candidates {
		if p.Hidden || workstreamGroupOff(p, groupOff) {
			continue
		}
		out = append(out, p)
	}
...
func conflictIDs(matches []Workstream) []string {
...
// projectGroupOff.
// NOTE: this is the ONLY converter from settings.RemoteWorkstream. internal/atlas
...
func FromRemoteWorkstreams(values []settings.RemoteWorkstream) []Workstream {
	out := make([]Workstream, 0, len(values))
	for _, v := range values {
		...
		// pinned by TestOneWorkstreamNamingTheSameRepoTwiceAttributesRatherThanConflicts
		...
		out = append(out, Workstream{
			...
			// `team` is the ONLY bucket Atlas serves (settings.RemoteWorkstream has
...
func Attribute(dims map[string]enrich.Labeled, candidates []Workstream, groupOff func(key string) bool, vector Vector) Result {
	visible := Visible(candidates, groupOff)

	if repo, ok := attributedValue(dims, DimRepo); ok {
		var matches []Workstream
		...
		if len(matches) == 1 {
			return Result{WorkstreamID: matches[0].ID, Method: MethodRepo}
		}
		if len(matches) > 1 {
	...
	if branch, ok := attributedValue(dims, DimBranch); ok {
		if key, ok := ticketKeyIn(branch); ok {
			...
			if len(matches) == 1 {
				return Result{WorkstreamID: matches[0].ID, Method: MethodTicket}
			}
			if len(matches) > 1 {
				return Result{Reason: ReasonConflict, Conflict: conflictIDs(matches)}
			}
...
func MergeCandidates(local, remote []Workstream) []Workstream {
	overlay := map[string]Workstream{}
	used := map[string]bool{}
	for _, l := range local {
		overlay[l.ID] = l
	}
	out := make([]Workstream, 0, len(remote)+len(local))
