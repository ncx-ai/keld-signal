...
//
// The store-side twin is TestOneWorkstreamNamingARepoTwiceIsNotAConflict in
// internal/agent/ledger; this one is the matcher's half and is the one that can
// actually regress.
func TestOneWorkstreamNamingTheSameRepoTwiceAttributesRatherThanConflicts(t *testing.T) {
	p := Workstream{
	...
	if res.WorkstreamID != p.ID {
		t.Fatalf("project = %q, want %q", res.WorkstreamID, p.ID)
	}
...
func TestTwoDifferentWorkstreamsClaimingOneRepoIsStillAConflict(t *testing.T) {
	a := Workstream{ID: "keld_projects:a", Title: "A", Repos: []string{repoKeldSignal}, Group: "development"}
	b := Workstream{ID: "keld_projects:b", Title: "B", Repos: []string{repoKeldSignal}, Group: "development"}
	dims := dimsWith(map[string]string{DimRepo: repoKeldSignal})
	...
	if res.WorkstreamID != "" {
		t.Fatalf("a conflict silently chose %q", res.WorkstreamID)
	}
}
