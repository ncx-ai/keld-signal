...
func TestARepoDeclaredTwiceByTheOrgIsImportedOnce(t *testing.T) {
	got := FromRemoteWorkstreams([]settings.RemoteWorkstream{{
...
func TestAWorkstreamWithNoDuplicatesIsUnchanged(t *testing.T) {
	in := []string{repoKeldSignal, repoKeldAtlas}
	got := FromRemoteWorkstreams([]settings.RemoteWorkstream{{ID: "keld_projects:x", Repos: in}})
...
func TestADedupedWorkstreamStillAttributes(t *testing.T) {
	got := FromRemoteWorkstreams([]settings.RemoteWorkstream{{
		ID:    "keld_projects:signal_on_device_client",
		Repos: []string{repoKeldSignal, repoKeldSignal},
		Team:  "development",
	...
	if res.WorkstreamID != "keld_projects:signal_on_device_client" {
		t.Fatalf("project = %q, want the deduped project to still match", res.WorkstreamID)
	}
