...
func TestEvidenceForSummarisesOnlyAttributedBlocksNeverAsARule(t *testing.T) {
	blocks := []AttributedBlock{
		{WorkstreamID: "p1", Dims: dimsWith(map[string]string{
		...
		{WorkstreamID: "p2", Dims: dimsWith(map[string]string{
...
func TestEvidenceForUnknownWorkstreamIsEmpty(t *testing.T) {
	blocks := []AttributedBlock{{WorkstreamID: "p1", Dims: dimsWith(map[string]string{DimLanguage: "go"})}}
