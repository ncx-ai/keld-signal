...
func TestHideExcludesFromMatchingButKeepsRules(t *testing.T) {
	doc := Document{Workstreams: []Workstream{{ID: "p1", Title: "One", Repos: []string{repoKeldSignal}}}}
	doc, err := Hide(doc, "p1", true)
	if err != nil {
		t.Fatalf("Hide: %v", err)
	}
	if !doc.Workstreams[0].Hidden {
		t.Fatalf("Hidden not set: %+v", doc.Workstreams[0])
	}
	if len(doc.Workstreams[0].Repos) != 1 {
		t.Fatalf("rules dropped by Hide: %+v", doc.Workstreams[0])
	}
...
func TestEditsOnUnknownWorkstreamFail(t *testing.T) {
	doc := Document{}
	if _, err := AddRules(doc, "missing", []Rule{{Kind: RuleKindRepo, Value: repoKeldSignal}}); !errors.Is(err, ErrWorkstreamNotFound) {
		t.Fatalf("AddRules on missing project: err = %v, want ErrWorkstreamNotFound", err)
	}
	if _, err := RemoveRules(doc, "missing", []Rule{{Kind: RuleKindRepo, Value: repoKeldSignal}}); !errors.Is(err, ErrWorkstreamNotFound) {
		t.Fatalf("RemoveRules on missing project: err = %v, want ErrWorkstreamNotFound", err)
	}
	if _, err := Hide(doc, "missing", true); !errors.Is(err, ErrWorkstreamNotFound) {
		t.Fatalf("Hide on missing project: err = %v, want ErrWorkstreamNotFound", err)
	}
}
...
func TestPlaceSameAsAddsRuleAndReattributes(t *testing.T) {
	doc := Document{Workstreams: []Workstream{{ID: "p1", Title: "One"}}}
	...
	if len(doc.Workstreams[0].Repos) != 1 || doc.Workstreams[0].Repos[0] != normalizeRepo(repoKeldSignal) {
		t.Fatalf("repo rule not added: %+v", doc.Workstreams[0])
	}
