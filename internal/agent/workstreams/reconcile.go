...
func Reconcile(d Document, remote []Workstream, groupOff func(key string) bool) (Document, []Removed, []Trimmed) {
	...
	var removed []Removed
	var trimmed []Trimmed
	out := make([]Workstream, 0, len(d.Workstreams))
	for _, p := range d.Workstreams {
...
func coveredRules(remote []Workstream, groupOff func(key string) bool) map[string]bool {
	out := map[string]bool{}
