...
// ErrWorkstreamNotFound is returned by every edit that targets a project id the
// document does not have.
var ErrWorkstreamNotFound = fmt.Errorf("projects: project not found")
...
func findWorkstream(d Document, id string) (int, bool) {
	for i := range d.Workstreams {
		if d.Workstreams[i].ID == id {
			return i, true
		}
	}
	return -1, false
...
// newWorkstreamID mints a human-legible, unique id from title: "p_" + a slug of
...
// collide).
func newWorkstreamID(d Document, title string) string {
	base := "p_" + slugify(title)
	id := base
	for n := 2; ; n++ {
		if _, ok := findWorkstream(d, id); !ok {
			return id
		}
...
func Bundle(d Document, title, group string, suggestionIDs []string, suggestions []Suggestion) (Document, Workstream, error) {
	p := Workstream{
		Title:  title,
		Group:  group,
		Origin: OriginUser,
	...
	for _, sid := range suggestionIDs {
		...
		if !ok {
			return d, Workstream{}, fmt.Errorf("%w: %s", ErrUnknownSuggestion, sid)
		}
	...
	p.ID = newWorkstreamID(d, title)
	next := d
	next.Workstreams = append(append([]Workstream(nil), d.Workstreams...), p)
...
func AddRules(d Document, workstreamID string, add []Rule) (Document, error) {
	i, ok := findWorkstream(d, workstreamID)
	if !ok {
		return d, ErrWorkstreamNotFound
	}
	next := d
	next.Workstreams = append([]Workstream(nil), d.Workstreams...)
	p := next.Workstreams[i]
	for _, r := range add {
		applyRule(&p, r, true)
	}
	next.Workstreams[i] = p
	return next, nil
}
...
func RemoveRules(d Document, workstreamID string, remove []Rule) (Document, error) {
	i, ok := findWorkstream(d, workstreamID)
	if !ok {
		return d, ErrWorkstreamNotFound
	}
...
func Hide(d Document, workstreamID string, hidden bool) (Document, error) {
	i, ok := findWorkstream(d, workstreamID)
	if !ok {
		return d, ErrWorkstreamNotFound
	}
	next := d
	next.Workstreams = append([]Workstream(nil), d.Workstreams...)
	next.Workstreams[i].Hidden = hidden
	return next, nil
}
...
func SameAsCandidates(d Document, groupOff func(key string) bool) []Workstream {
...
func SameAsCandidatesWithRemote(d Document, remote []Workstream, groupOff func(key string) bool) []Workstream {
	return Visible(MergeCandidates(d.Workstreams, remote), groupOff)
}
...
// (ErrWorkstreamNotFound) when hidden or missing — both cases SameAsCandidates
// already excludes, so a caller that only offers those candidates cannot hit
// either refusal by surprise.
func PlaceSameAs(d Document, suggestionID, targetWorkstreamID string, suggestions []Suggestion, groupOff func(key string) bool) (Document, error) {
	return PlaceSameAsWithRemote(d, nil, suggestionID, targetWorkstreamID, suggestions, groupOff)
}
...
func PlaceSameAsWithRemote(d Document, remote []Workstream, suggestionID, targetWorkstreamID string, suggestions []Suggestion, groupOff func(key string) bool) (Document, error) {
	s, ok := findSuggestion(suggestions, suggestionID)
	if !ok {
		return d, fmt.Errorf("%w: %s", ErrUnknownSuggestion, suggestionID)
	}
	i, ok := findWorkstream(d, targetWorkstreamID)
	if !ok {
		// Not local. If it is one of the org's values, lay an overlay entry
		// down for it; otherwise it genuinely does not exist.
		var rv *Workstream
		for k := range remote {
			if remote[k].ID == targetWorkstreamID {
				rv = &remote[k]
				break
			}
		}
		...
		if workstreamGroupOff(*rv, groupOff) {
			return d, ErrGroupOff
		}
		d.Workstreams = append(append([]Workstream(nil), d.Workstreams...), Workstream{
			ID:     rv.ID,
			Title:  rv.Title,
			Team:   rv.Team,
			Group:  rv.Group,
		...
		i = len(d.Workstreams) - 1
	}
	target := d.Workstreams[i]
	if target.Hidden {
		return d, ErrWorkstreamNotFound
	}
	if workstreamGroupOff(target, groupOff) {
		return d, ErrGroupOff
	}
	next := d
	next.Workstreams = append([]Workstream(nil), d.Workstreams...)
...
// MapWorkstreamTo folds a LOCAL project into another project — normally one of the
...
func MapWorkstreamTo(d Document, remote []Workstream, localWorkstreamID, targetWorkstreamID string,
	groupOff func(key string) bool) (Document, error) {
	if localWorkstreamID == "" || targetWorkstreamID == "" {
		return d, ErrWorkstreamNotFound
	}
	...
	li, ok := findWorkstream(d, localWorkstreamID)
	if !ok {
		return d, ErrWorkstreamNotFound
	}
	src := d.Workstreams[li]
	...
	ti, ok := findWorkstream(next, targetWorkstreamID)
	if !ok {
		var rv *Workstream
		for k := range remote {
			if remote[k].ID == targetWorkstreamID {
		...
		next.Workstreams = append(next.Workstreams, Workstream{
			ID:     rv.ID,
			Title:  rv.Title,
			Team:   rv.Team,
			Group:  rv.Group,
		...
		ti = len(next.Workstreams) - 1
	}
	target := next.Workstreams[ti]
	if target.Hidden {
		return d, ErrWorkstreamNotFound
	}
	if workstreamGroupOff(target, groupOff) {
	...
	out := next.Workstreams[:0:0]
	for _, x := range next.Workstreams {
		if x.ID == localWorkstreamID {
			continue
		}
		out = append(out, x)
	}
	next.Workstreams = out
	return next, nil
}
