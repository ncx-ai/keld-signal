...
func TestRepoRuleAttributesWithoutAnyEncoder(t *testing.T) {
	p := Workstream{ID: "p_signal", Title: "Keld Signal", Repos: []string{repoKeldSignal}, Group: "development"}
	dims := dimsWith(map[string]string{DimRepo: repoKeldSignal})

	res := Attribute(dims, []Workstream{p}, nil, nil)

	if res.Method != MethodRepo {
		t.Fatalf("method = %q, want %q", res.Method, MethodRepo)
	}
	if res.WorkstreamID != "p_signal" {
		t.Fatalf("project id = %q, want p_signal", res.WorkstreamID)
	}
...
func TestSuggestBundleRemoveRoundTrip(t *testing.T) {
	...
	for i := 0; i < 4; i++ {
		...
		if res.WorkstreamID != "" || res.Reason != ReasonNoRuleMatched {
	...
	if len(doc.Workstreams) != 1 {
		t.Fatalf("document has %d projects, want 1", len(doc.Workstreams))
	}

	// All three repos' blocks now re-attribute to the new project.
	for _, r := range sdkRepos {
		res := Attribute(sdkDims[r], doc.Workstreams, nil, nil)
		if res.Method != MethodRepo || res.WorkstreamID != created.ID {
	...
	res := Attribute(sdkDims[repoSDKTestbench], doc.Workstreams, nil, nil)
	if res.WorkstreamID != "" {
...
// silently the first.
func TestTwoWorkstreamsClaimingOneRepoConflicts(t *testing.T) {
	a := Workstream{ID: "p_a", Title: "A", Repos: []string{repoKeldAtlas}}
	b := Workstream{ID: "p_b", Title: "B", Repos: []string{repoKeldAtlas}}
	dims := dimsWith(map[string]string{DimRepo: repoKeldAtlas})

	res := Attribute(dims, []Workstream{a, b}, nil, nil)

	if res.Reason != ReasonConflict {
		t.Fatalf("reason = %q, want conflict: %+v", res.Reason, res)
	}
	if res.WorkstreamID != "" {
		t.Fatalf("a conflict must not silently name a project, got %q", res.WorkstreamID)
	}
	want := []string{"p_a", "p_b"}
	sort.Strings(want)
	...
	res2 := Attribute(dims, []Workstream{b, a}, nil, nil)
...
func TestGroupOffExcludesFromMatchingAndSameAs(t *testing.T) {
	...
	p := Workstream{ID: "p_mkt", Title: "Marketing site", Repos: []string{repoKeldSignal}, Group: "marketing"}
	dims := dimsWith(map[string]string{DimRepo: repoKeldSignal})

	res := Attribute(dims, []Workstream{p}, off, nil)
	if res.WorkstreamID != "" || res.Reason != ReasonNoRuleMatched {
		t.Fatalf("workstream-off project still matched: %+v", res)
	}

	same := SameAsCandidates(Document{Workstreams: []Workstream{p}}, off)
	for _, c := range same {
		if c.ID == p.ID {
	...
	doc := Document{Workstreams: []Workstream{p}}
...
func TestEvidenceFieldsAreNeverMatchInputs(t *testing.T) {
	p := Workstream{ID: "p1", Repos: []string{"go", "git", "myworkspace"}, TicketKey: "GIT"}
	...
	for name, dims := range cases {
		t.Run(name, func(t *testing.T) {
			res := Attribute(dims, []Workstream{p}, nil, nil)
			if res.WorkstreamID != "" || res.Method != MethodNone {
...
func TestRemoteWorkstreamAttributesByRepoShapedKeyword(t *testing.T) {
	values := []settings.RemoteWorkstream{
		{ID: "v_web", Title: "Web", Team: "Development", Keywords: []string{"ncx-ai/keld-signal"}},
	}
	candidates := FromRemoteWorkstreams(values)
	dims := dimsWith(map[string]string{DimRepo: repoKeldSignal})

	res := Attribute(dims, candidates, nil, nil)
	if res.Method != MethodRepo || res.WorkstreamID != "v_web" {
...
func TestRemoteWorkstreamWithNoRepoLikeKeywordsNeverAttributesByRepo(t *testing.T) {
	values := []settings.RemoteWorkstream{
...
func TestConflictOnlyFromRealRepoMatchesNeverFromSharedFreeTags(t *testing.T) {
	a := Workstream{ID: "p_a", Title: "A", Keywords: []string{"design/ux"}}
	b := Workstream{ID: "p_b", Title: "B", Keywords: []string{"design/ux"}}
	dims := dimsWith(map[string]string{DimRepo: repoKeldSignal})

	if res := Attribute(dims, []Workstream{a, b}, nil, nil); res.Reason == ReasonConflict {
		t.Fatalf("two projects sharing a non-repo keyword conflicted: %+v", res)
	}

	c := Workstream{ID: "p_c", Title: "C", Keywords: []string{"ncx-ai/keld-signal"}}
	d := Workstream{ID: "p_d", Title: "D", Keywords: []string{"ncx-ai/keld-signal"}}
	if res := Attribute(dims, []Workstream{c, d}, nil, nil); res.Reason != ReasonConflict {
...
func TestRemovingAMatchedKeywordRuleReturnsBlocksToSuggestions(t *testing.T) {
	doc := Document{Workstreams: []Workstream{{ID: "p1", Title: "One", Keywords: []string{"ncx-ai/keld-signal"}}}}
	dims := dimsWith(map[string]string{DimRepo: repoKeldSignal})

	res := Attribute(dims, doc.Workstreams, nil, nil)
	if res.Method != MethodRepo || res.WorkstreamID != "p1" {
		t.Fatalf("expected attribution via matched keyword candidate, got %+v", res)
	}
	if got := Rules(doc.Workstreams[0], []string{repoKeldSignal}); len(got) != 1 {
	...
	res2 := Attribute(dims, doc.Workstreams, nil, nil)
	if res2.WorkstreamID != "" {
	...
	if got := Rules(doc.Workstreams[0], []string{repoKeldSignal}); len(got) != 0 {
