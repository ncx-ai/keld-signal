...
// already set, which is a shape Atlas never sends — `settings.RemoteWorkstream`
...
func atlasValue(id, title, team string) settings.RemoteWorkstream {
	return settings.RemoteWorkstream{ID: id, Title: title, Team: team}
}

func TestARemoteWorkstreamCarriesItsBucketKey(t *testing.T) {
	// The wire shape: a team, and no workstream field to read.
	ps := FromRemoteWorkstreams([]settings.RemoteWorkstream{
...
func TestAWorkstreamAndItsBucketDeriveTheSameKey(t *testing.T) {
	...
	for _, team := range []string{
		...
		p := FromRemoteWorkstreams([]settings.RemoteWorkstream{atlasValue("x", "X", team)})[0]
...
func TestARemoteWorkstreamWithNoTeamGetsNoBucketRatherThanAWrongOne(t *testing.T) {
	...
	p := FromRemoteWorkstreams([]settings.RemoteWorkstream{atlasValue("x", "X", "")})[0]
	if p.Group != "" {
