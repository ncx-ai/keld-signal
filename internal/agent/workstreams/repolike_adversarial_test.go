...
func TestProseTagsNeverBecomeRules(t *testing.T) {
	...
	p := Workstream{ID: "p1", Title: "Marketing", Keywords: prose}
	...
	for _, s := range prose {
		if len(MatchedCandidates(Workstream{Keywords: []string{s}}, observed)) != 0 {
...
func TestARealTagBecomesARuleOnlyOnceItHasMatched(t *testing.T) {
	p := Workstream{ID: "p1", Title: "Web", Keywords: []string{"acme/web"}}
