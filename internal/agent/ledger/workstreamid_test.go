...
func TestALocalWorkstreamIDIsUnaffected(t *testing.T) {
	setHome(t)
	s := New()
	k := BlockKey{Session: "9eb2b3ffbbbb2222", Start: 1788600000}
	s.Cut(k, 1788601200, "idle", "budget", "claude_code", time.Now())
	s.Attribute(k, Attributed{WorkstreamID: "p_github_com_ncx_ai_keld_signal", Method: MethodRepo}, ReasonNone, time.Now())
...
func TestOnlyTheColonWasAdmitted(t *testing.T) {
	...
	for name, id := range bad {
		t.Run(name, func(t *testing.T) {
			...
			s.Cut(k, 1788601200, "idle", "budget", "claude_code", time.Now())
			s.Attribute(k, Attributed{WorkstreamID: id, Method: MethodRepo}, ReasonNone, time.Now())
...
func TestAnAttributionCanNeverBeOKWithNoWorkstream(t *testing.T) {
	setHome(t)
	s := New()
	...
	s.Attribute(k, Attributed{WorkstreamID: "", Method: MethodRepo}, ReasonNone, time.Now())
...
func TestAFailedAttributionWithNoWorkstreamIsStillRecorded(t *testing.T) {
	setHome(t)
	s := New()
...
func TestAConflictBetweenAtlasWorkstreamsKeepsBothIDs(t *testing.T) {
	setHome(t)
	s := New()
