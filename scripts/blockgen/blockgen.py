...
# =================================================================== the exported corpus
#
# ⚠️ **THE DAEMON GENERATES BLOCKS TOO, AND IT CANNOT RUN THIS SCRIPT.**
# Signal's Settings pane has a developer "Generate block" button, and the block it produces has
# to be a REAL block — a JSONL transcript in a watched root, ingested by the sidecar, cut by the
# real cutter, priced by the real table — or it proves nothing about the pipeline it exists to
# exercise. But the daemon is a single Go binary: a frozen install ships no `scripts/` directory
# at all, and the service is started by launchd/systemd with no venv on its PATH, so shelling
# out to this file works on exactly one machine (a developer's checkout) and fails on every
# other, silently, in the direction that looks like "the button does nothing".
#
# So the split is: this module stays the SOURCE OF TRUTH for what generated work says, and the
# Go side owns writing the lines. `--emit-corpus` dumps the vocabularies below to JSON; that file
# is committed under internal/agent/devgen/ and `go:embed`ed. `test_blockgen.py` asserts the
# committed copy still matches these constants, so the two cannot drift without a test failing —
# which is the whole reason the corpus is exported rather than retyped in Go.
#
# What is NOT exported is the session SHAPING (run/break structure, WRR mixes, token curves).
# The daemon writes one short session on demand; this script writes a corpus spanning days. They
# are different jobs, and pretending otherwise would mean exporting a program rather than data.

def corpus_document():
    """The text and file vocabularies, as the plain-data document the Go side embeds."""
    return {
        "note": ("Generated by scripts/blockgen/blockgen.py --emit-corpus. "
                 "Do not hand-edit: test_blockgen.py asserts this matches the module."),
        "dev_verbs": DEV_VERBS,
        "dev_objects": DEV_OBJECTS,
        "dev_conditions": DEV_CONDITIONS,
        "prompt_templates": PROMPT_TEMPLATES,
        "response_templates": RESPONSE_TEMPLATES,
        "lang_files": LANG_FILES,
        "primary_marker": PRIMARY_MARKER,
        "bash_templates": BASH_TEMPLATES,
        "grep_patterns": GREP_PATTERNS,
        "branch_slugs": BRANCH_SLUGS,
        "models": [m[0] for m in DEFAULT_PROFILE["models"]],
        "turns_per_prompt": DEFAULT_PROFILE["turns_per_prompt"],
        "tokens_per_active_minute": DEFAULT_PROFILE["tokens_per_active_minute"],
        # The repositories the daemon offers as its DEFAULT stable set. It reads the operator's
        # own list from settings and only falls back to this, so the two lists agree out of the
        # box without the Go side hard-coding org names of its own.
        "default_repositories": [
            {"remote": r["remote"], "workspace": r["workspace"],
             "language": r["language"], "ticket_prefix": r["ticket_prefix"]}
            for r in DEFAULT_PROFILE["repositories"][:3]
        ],
    }


def emit_corpus(path):
    doc = corpus_document()
...
def parse_args(argv=None):
    ...
    p.add_argument("--out", help="output directory (required unless --emit-corpus is given)")
    p.add_argument("--emit-corpus", default=None, metavar="PATH",
                   help="write the text corpus the daemon embeds (internal/agent/devgen/"
                        "corpus.json) and exit. Nothing else is generated.")
...
def main(argv=None):
    ...
    if args.emit_corpus:
        ...
        print("corpus -> %s" % args.emit_corpus)
        return 0
    if not args.out:
        raise SystemExit("--out is required (or use --emit-corpus)")
