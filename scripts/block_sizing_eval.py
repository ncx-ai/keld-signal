...
def sweep(store=None):
    ...
    share, block-duration percentiles, blocks per session, the share of blocks clearing
    `MIN_EVIDENCE`, and the share of SESSIONS whose every boundary was `budget` (those sessions
    measure the cap and nothing else, per BLOCK-SIZING-PREREGISTRATION.md).
    ...
    per_cap = {cap: {"n_blocks": 0, "n_sessions": 0, "budget": 0, "detected": 0,
                     "durations": [], "evidence_clear": 0, "all_budget_sessions": 0}
              for cap in CAPS}
    ...
    for s in sessions:
        ...
        for cap in CAPS:
            ...
            # share rather than counted as vacuously true.
            ...
            for bl in blocks:
                ...
                if bl.end_reason in ("budget", "detected"):
                    acc[bl.end_reason] += 1
                if block_evidence(st, s, bl) >= MIN_EVIDENCE:
                    acc["evidence_clear"] += 1
        if hasattr(st, "reset"):
            st.reset()
    ...
    for cap in CAPS:
        ...
        rows.append({
            ...
            "evidence_clear_share": 100.0 * acc["evidence_clear"] / n_blocks if n_blocks else 0.0,
