 ...
 * needs local projects and suggestions to exist. Left in workstreams.spec.ts they
...
test.describe("Map a local project", () => {
  test('THE STORY: mapping a local project onto another moves its rules and removes it',
    async ({ signal, page }) => {
      // Runs after "New workstream" in this serial file, so there is a local
      ...
      const local = (before.workstreams || []).filter(
      ...
      const picker = page.getByLabel(`Map ${src.title} onto another workstream`);
      ...
      const target = (after.workstreams || []).find((p: any) => p.id === dst.id);
      expect(target).toBeTruthy();
      for (const repo of src.repos) expect(target.repos).toContain(repo);
      expect((after.workstreams || []).some((p: any) => p.id === src.id)).toBe(false);
      ...
      await expect(page.locator(".workstream-row").getByText(src.title, { exact: true }))
        .toHaveCount(0);
    });
  ...
  test('NEGATIVE: an Atlas project offers no "Same as" — it is not ours to fold away',
    async ({ signal, page }) => {
      ...
      const org = (d.workstreams || []).filter((p: any) => p.origin === "atlas");
      test.skip(org.length === 0, "no org projects on this daemon (Send to Atlas is off)");
      for (const p of org.slice(0, 3)) {
        await expect(page.getByLabel(`Map ${p.title} onto another workstream`))
          .toHaveCount(0);
      }
    });
});
