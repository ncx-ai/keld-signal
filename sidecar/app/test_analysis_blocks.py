...
# --- covers is DELETED, and this section is what stops it coming back ---------------------------
#
# `covers` mapped a block to the prompt episodes overlapping it. It is gone, and the argument is
# not "it was broken" (though it was -- `watch/filter.go` yields `promptId` while `ingest.py`
# indexes the per-message `uuid`, so `store.prompt_time` resolved NONE of them and every real run
# published `covers: []`). It is that a block is TIME, end to end: a cap, a silence threshold, a
# rollup over a span. Atlas must join `event.ts in [block.start, block.end)` within the session
# for COST anyway -- a turn spanning several blocks would double-count spend through `covers` --
# and that same join answers the display question, because Atlas holds `ToolEvent.session_id` and
# `event_ts`. So `covers` was a second, weaker copy of a join Atlas owes regardless.
#
# A block is now exactly (principal, session, span, boundary reasons, facets).

def test_the_cutter_has_no_covers_function():
    """The deletion, pinned at the module. A cutter that grew this back would be reintroducing a
    prompt-id space into a unit that has none -- see the section comment above."""
    assert not hasattr(blocks, "covers"), \
        "blocks.covers is back: a block is a span, not a prompt mapping"


def test_the_cutter_names_no_prompt_at_all():
    """Wider than the name. `covers` was the only thing in this module that knew a prompt existed,
    so the honest pin is that nothing here reads or returns one -- not that one function is gone."""
    import inspect
    for name, obj in vars(blocks).items():
        if not inspect.isfunction(obj) or obj.__module__ != blocks.__name__:
            continue
        params = list(inspect.signature(obj).parameters)
        assert not any("prompt" in p for p in params), \
            "blocks.%s takes a prompt argument: %r" % (name, params)


def test_a_block_carries_four_fields_and_none_of_them_is_a_prompt():
    """`Block` is the whole unit: two instants and two boundary reasons. Nothing else."""
    assert list(blocks.Block._fields) == ["start", "end", "start_reason", "end_reason"], \
        blocks.Block._fields
