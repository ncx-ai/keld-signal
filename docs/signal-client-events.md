...
| `spool.drain_failed` | warn | `daemon/daemon.go` | A periodic spool sweep's `spool.Drain` call returned an error (backend wedged: disk full, EACCES, a corrupt page, etc.) instead of its pre-SQLite-era guarantee of always returning nil. `fields.error`. Under a completeness SLO a silently-discarded error here is uniquely dangerous: the same failing backend also fails `Stats()`, so a wedged spool would otherwise produce no drains, no `spool.depth` gauge, and no event at all — this is deliberately the one signal that still gets out. |
