...
| `features.encoder_provisioning` | warn (floor-exempt) | `daemon/encoder_on_demand.go` | The text encoder's weights (`Qwen/Qwen3-Embedding-0.6B`, ~1.2 GB) are being fetched, on demand, because a transcript advanced while both `KELD_TEXTEMBED` and the `features` toggle were on. `fields.repo`, `fields.revision`, `fields.attempt`. **Nothing waits on this fetch** — the sidecar, `/features`, `/analyze`, `/ingest`, `/blocks` and `/pii` all answer throughout, and a feature row's text half reports `degraded:weights_unavailable` until the weights land. Emitted **before** the download, because the whole point of the three codes is that an operator can tell "fetching" from "never started". |
| `features.encoder_provisioned` | info (floor-exempt) | `daemon/encoder_on_demand.go` | The encoder weights are installed and verified; the sidecar's next textembed retry fills the text half. `fields.duration_s`, `fields.attempt`. Floor-exempt deliberately: under the default `warn` floor this would be dropped and only failures would ever appear, leaving "ready" indistinguishable from "never started". |
