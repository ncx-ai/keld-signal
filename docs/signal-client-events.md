...
| `sidecar.unavailable` | warn (pre-supervisor) or error (supervisor-level) | `daemon/daemon.go` (missing binary or port-alloc failure, warn) and `daemon/supervisor.go` (spawn/start failure or restart cap exceeded, error) | The sidecar could not be brought up at all for this daemon run: no sidecar binary present, no ephemeral port, `exec.Cmd` build/start failure, or the restart cap was exceeded. Enrichment is ML-only and does not degrade to any other backend — the readiness gate stays closed and jobs queue/spool (then quarantine after `KELD_ENRICH_MAX_ATTEMPTS`) until the sidecar comes up on a restart. `fields.error` (or `fields.restarts` for cap-exceeded). (Formerly `sidecar.fallback`, renamed 2026-07-14 — the deterministic fallback it implied no longer exists; envelope shape unchanged, no `schema_version` bump.) |
...
| `service.stuck` | error | `daemon/servicehealth.go` | Restarting the analysis service and then the daemon did not fix it; nothing further will be restarted automatically. `fields.failures`, `fields.sidecar_restarts`, `fields.daemon_restarted`, `fields.supervisor_gave_up`. ⚠️ This is the code that distinguishes a broken machine from a quiet user, and it is the whole reason the ladder COUNTS: what reaches a person has to be "restarting twice did not fix this", not "something broke". Emitted once per streak, not once per probe. |
