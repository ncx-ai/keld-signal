...
| `sidecar.fallback` | warn (pre-supervisor) or error (supervisor-level) | `daemon/daemon.go` (port-alloc failure, warn) and `daemon/supervisor.go` (spawn/start failure or restart cap exceeded, error) | The sidecar could not be brought up at all for this daemon run: no ephemeral port, `exec.Cmd` build/start failure, or the restart cap was exceeded. Enrichment does not degrade to any other backend — the readiness gate stays closed and jobs queue/spool (then quarantine after `KELD_ENRICH_MAX_ATTEMPTS`) until the sidecar comes up on a restart. `fields.error` (or `fields.restarts` for cap-exceeded). |
