...
- **CPU thread scaling (added post-plan):** spatial CPU lever beside the temporal governor. `CpuScaler` (`app/cpuscale.py`) maps host-load EWMA → torch intra-op thread count (idle ⇒ `KELD_SIDECAR_MAX_THREADS` (default `max(1, cpu_count // 2)`), saturated ⇒ `KELD_SIDECAR_MIN_THREADS`=1, ramp between over the `KELD_GOV_LOW`/`HIGH` marks; `KELD_SIDECAR_CPU_SCALE_DISABLED`). Applied via `torch.set_num_threads` in a `pre_run` hook the runner calls before each single-flight inference; surfaced as `governor.cpu_threads` in `/metrics`. Unit-tested in `app/test_cpuscale.py`.
