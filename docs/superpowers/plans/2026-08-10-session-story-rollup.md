...
- ~~Existing caps unchanged in this plan: `DefaultSynopsisCap 650`, `DefaultProseCap 900`, `DefaultHappenedCap 1400`, `DefaultStructureCap 1600`, `DefaultListCap 12`.~~ **SUPERSEDED after the sweep.** All four prose caps are **deleted**, constants and enforcement: with no prior prose embedded in a prompt, nothing in a prompt path depended on them (`Identifiers(prev)` is bounded by `boundRetainList`, the open-item accounting by `DefaultListCap` + `promptOpenItemCap`), so they only clipped what a person reads. `DefaultListCap 12` and `DefaultListEntryCap 300` remain. Measured effect on retention: **none** — see Part 8 of `2026-08-07-conversational-dimensions-findings.md`. Stating length in the prompt instead was tried in two wordings and **reverted**: the first lost two digests to an empty `unresolved`, the second took T9 from 1 flagged report to 6/9. Nothing now states a prose length except the synopsis's "Three or four sentences", and nothing enforces one.
