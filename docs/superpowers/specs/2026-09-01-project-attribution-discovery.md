...
| Worker lifecycle machinery | `sidecar/app/worker_manager.py`, `worker.py` | reuse: the verifier gets its OWN `WorkerManager`/child (⚠️ **not** `runner.py`/`governor.py` — see §6b correction) |
...
* **Verifier:** Gemma 4 E2B Q4_K_M via llama-cpp-python. Runs in **its OWN recycled worker child, under its OWN `WorkerManager`** — a second, independent instance beside GLiNER2's, with its own spawn, its own RSS margin (`KELD_VERIFIER_RSS_MARGIN_MB`) and its own recycle schedule. ON when attribution is on; `KELD_ATTRIBUTION_VERIFIER=0` opts out (slow machines). Skipping degrades, never blocks.
  ⚠️ **CORRECTED (task 6b). This line, and the reuse-ledger row above it, described the pre-6b design and were mutually contradictory** — "the recycled worker child" is GLiNER2's, which is not "under the runner's single-flight" in any sense that admits a second model, and the ledger row claimed `runner.py`/`governor.py` were reused for the verifier's CPU pacing. Neither is true as built: `runner.py`'s single-flight is GLiNER2's dispatch queue and putting the verifier in it would have made a multi-second llama.cpp verdict block every enrichment inference behind it. What is actually reused is `worker_manager.py` + `worker.py` — the spawn/recycle/RSS machinery — instantiated a SECOND time. Left as a correction rather than a silent edit because a stale design doc is how the wrong thing gets rebuilt.
  ⚠️ **Its manager must be POLLED, and initially was not.** `WorkerManager.poll()` is the sole driver of the RSS ceiling, the recycle, the idle unload and the pressure eviction; reusing the spawn half alone gives an unbounded child. It rides `lifespan`'s existing 1 s poll loop and reports under `/metrics.verifier`.
  ⚠️ **`llama_cpp` is a compiled native dependency and must be named in the PyInstaller spec** (`collect_all` + `collect_dynamic_libs` + hidden imports). It is a ctypes binding whose libraries are opened by a runtime-computed path, imported lazily from inside PyArmor-encrypted bytecode — invisible twice over, and its absence fails only in the shipped binary. `make freeze-check` spawns the verifier child and demands a verdict.
