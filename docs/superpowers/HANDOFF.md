...
- **Multi-stage follow-on still open:** for RAW calls without provided metadata, a stage-1 GLiNER pass could DERIVE the (helpful, selective) actor/intent from the system prompt. But raw already classifies ~well and the external metadata is ~neutral for raw, so the ROI is smaller than expected — deprioritized vs the clean-shape win just shipped.
